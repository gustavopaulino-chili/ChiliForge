...
LOGO COLOR STRATEGY (BINDING — controls the HEADER/NAV bar surface so the logo is always legible): ${(() => { const s = data.logoStrategy || 'auto'; const map: Record<string,string> = {
  'auto': 'Pick the header/nav surface (a LIGHT or DARK shade from the brand palette) that gives the logo MAXIMUM contrast, and treat the logo to match.',
  'light': 'The logo is LIGHT/white. THEREFORE the HEADER/NAV bar MUST have a DARK surface (use the brand primary or a dark brand shade). It is FORBIDDEN to put the light logo on a light header. Render the logo white (filter brightness(0) invert(1) if needed).',
  'dark': 'The logo is DARK. THEREFORE the HEADER/NAV bar MUST have a LIGHT surface (brand background or a light brand shade). ⛔ DARK COLORS ARE FORBIDDEN in the header/nav — never use a dark/near-black header background. Render the logo in its dark color, no inversion.',
  'monochrome': 'Render the logo MONOCHROME (grayscale). Choose a header surface (light or dark brand shade) with strong contrast to the logo.',
  'full-color': 'Keep the logo in FULL ORIGINAL COLOR (no filters). Choose a header surface that maximizes the logo contrast/legibility.'
