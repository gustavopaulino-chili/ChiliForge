...
import { Upload, FileSpreadsheet, Check, AlertCircle, Table, X, Loader2, Sparkles, Globe, MessageSquare } from 'lucide-react';
import { toast } from 'sonner';
import { parseSpreadsheet, scrapeWebsite } from '@/services/api';
...
async function sheetToText(sheet: WorkSheet): Promise<string> {
...
function normalizeColor(value: unknown): string | null {
  ...
  // Use a non-word-boundary regex to match even inside longer strings like "#3b82f6ff"
  ...
  if (hexMatch) {
    ...
    // 8-char RGBA — strip alpha (last 2 chars)
  ...
  // Convert rgba(r,g,b,a) to hex (drop alpha)
...
function aiDataToFormUpdates(extracted: Record<string, any>): Partial<BusinessFormData> {
  ...
  if (extracted.preferredStyle) {
    // Normalize scraped/AI style values to valid form enum
...
export function StepCsvImport({ data, onChange }: Props) {
  ...
  const processSheetWithAI = async (wb: WorkBook, sheetName: string) => {
    const sheet = wb.Sheets[sheetName];
    if (!sheet) return;

    const sheetText = await sheetToText(sheet);
    if (!sheetText.trim()) {
    ...
    try {
      const result = await parseSpreadsheet(sheetText, context || undefined);
  ...
  const handleScrapeWebsite = async () => {
    ...
    try {
      const result = await scrapeWebsite(websiteUrl.trim(), false, context || undefined);
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Contextualization field */}
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          <MessageSquare className="h-4 w-4 text-primary" />
          <Label className="text-sm font-semibold text-foreground">Context / Additional Description</Label>
          <span className="text-xs text-muted-foreground">(optional)</span>
        </div>
        <Textarea
          placeholder="Describe the business in your own words to help the AI interpret the data better. Example: 'This is a premium dental clinic in São Paulo focused on cosmetic dentistry for high-income adults. The brand should feel luxurious and modern.'"
          value={context}
          onChange={e => setContext(e.target.value)}
          className="min-h-[90px] text-sm"
        />
        <p className="text-xs text-muted-foreground">
          This description is used by the AI when analyzing the URL or spreadsheet — it helps fill in gaps and disambiguate information.
        </p>
      </div>
