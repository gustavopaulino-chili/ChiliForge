  ...
  try {
    ...
    const userMessage = [
      ...
      [
        ...
        "• LEAD-CAPTURE FORM IS MANDATORY: the page MUST include at least one real, visible <form> for lead capture (typically in a contact/CTA section). Give it labeled fields with placeholders — Name, Email, Phone (and an optional Message) — required where it makes sense, plus a full-width, high-contrast submit button with an action-oriented label. NEVER replace the form with a plain link or a mailto:. Use standard field names: name, email, phone, message. The form must always be present even if the section config didn't explicitly ask for one.",
