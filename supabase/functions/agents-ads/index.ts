...
const PRIMARY_BG_RULE =
  "███ RULE #1 — READ THIS FIRST, IT OVERRIDES EVERYTHING BELOW ███\nYou are producing ONLY the BACKGROUND of an advertisement — nothing else. You must NOT draw ANY text, letter, word, number, headline, caption, slogan, price, brand name, wordmark or logo ANYWHERE in the image — ZERO. The real logo and all copy are added by the system in a separate layer ON TOP of your image afterwards. Your whole job is ONE clean, cohesive, on-brand background scene that leaves calm space for that overlay. A background that contains ANY text or logo is a total failure and is discarded.\n⛔ THIS IS A PHOTOGRAPH/ILLUSTRATION, NOT A LAYOUT MOCKUP: do NOT draw any card, chip, banner, badge, callout bubble, sign or panel that CONTAINS lettering, a title, a label or a mark — and never leave an empty box, frame or circle standing in for one. The headline, and the name of the product, service, company or industry, are all supplied by us on a separate layer: the scene itself carries none of them, in ANY language, on ANY surface — not translated, not abbreviated, not stylised. ✅ ALLOWED and ENCOURAGED: blank brand-colour shapes, organic blobs, diagonal fields, panels and dot clusters as tasteful background DEVICES that make it look like this brand — as long as they carry ZERO lettering or mark-like graphics. The ban is only on lettering and mark-like graphics, NEVER on clean blank brand shapes. If you catch yourself composing a heading or a titled card, STOP.";
...
async function buildOverlayHtmlFromGemini(
  ...
  const calmZoneLine = calmZone
    ...
    ? `⛔ CALM-ZONE VERDICT — THIS IS MEASURED, NOT A SUGGESTION: a vision model analysed THIS exact background and found the emptiest, most text-safe region is the **${calmZone.toUpperCase()}**. ALL THREE elements — headline, subheadline AND CTA — live inside the ${calmZone} region. The HEADLINE sets the column: give it as much of the calm region's width as it needs to stay within its line limit — widen it before you ever let it wrap to a fourth line. The subheadline and the CTA then follow that column: same left edge, never wider than it, so they cannot run out of the calm region into a screen, a face or any detailed prop. Narrowing is for them, never for the headline. None of the three may sit over the main subject. If you choose OPTION A (split), the HEADLINE group stays in or next to the ${calmZone} region and the second group stays inside the other calm zone under the same width rule. If the ${calmZone} region looks tight, shrink the type or wrap earlier — never widen the column and never relocate it.`
    : "";
...
function buildBackgroundPrompt(
  ...
  if (bgSource === "reference" && hasRefImages) {
    ...
    if (brandPostsAreProxy) {
      sourceBlock = [
        ...
        "⭐ DECORATION — REINTERPRET, DON'T COPY (this is the fix for the 'pasted-on CSS-overlay' look): a graphic design-asset layer (floating dots, halftone, blobs, colour panels, rings, stars, badges) is welcome. Decoration is governed by PLACEMENT, not by a head count. Every device — dot cluster, blob, ring, halftone, colour panel — must live in the EMPTY margins: corners, a bare strip of wall, the edge of the frame. Four such pieces around a clean edge read as designed. Two pieces landing on the subject read as clutter. So: never over a face or the product, never over the headline or CTA, and never overlapping ANOTHER device. If a piece has nowhere empty to sit, drop it rather than shrink it. Read the brand posts for WHICH devices are theirs; their feed can be denser than this ad because a feed post has no headline competing for the eye. It is seasoning, not the dish: the photograph and the headline carry this ad, and every extra floating shape takes attention away from them. The competitor refs show you the VOCABULARY, never the quantity. BUT: (1) render every device in the CLIENT's colours (site / BRAND COLORS), NEVER the competitor's; (2) INTEGRATE it into the scene — grade it with the scene's light and depth so it feels native to this image, NOT a flat sticker layer floating disconnected on top; (3) do NOT slavishly 'camouflage into the competitor's feed' or match their exact devices/colours/density — you are building THIS brand's OWN identity USING the refs as inspiration, not reproducing the competitor. Keep it tasteful and legible: devices in empty areas only, ZERO text/letters/logo inside them.",
  ...
  if (heroRef && hasRefImages) {
    sourceBlock = [
      ...
      // the model the exact glyphs to draw: run 270 rendered the literal word from this very rule
      // even though only a colour swatch was attached (nothing to trace from), and run 276 burned
      // a wordmark beside the composited logo. Run 271, with no such enumeration, came out clean.
...
⛔ THE ATTACHED POST IMAGES ARE NOT THIS BRAND: they are market/category reference material showing how this KIND of business posts. Take ZERO colour from them — not their palette, not their grade, not their background tint.${hasSiteIdentity ? " The client's OWN website is among the attached images — that one, plus the colour named above, is the brand." : ""} Their value to you is structure and composition only.`
  ...
  return [
    ...
    "• Decoration is governed by PLACEMENT, not by a head count. Every device — dot cluster, blob, ring, halftone, colour panel — must live in the EMPTY margins: corners, a bare strip of wall, the edge of the frame. Four such pieces around a clean edge read as designed. Two pieces landing on the subject read as clutter. So: never over a face or the product, never over the headline or CTA, and never overlapping ANOTHER device. If a piece has nowhere empty to sit, drop it rather than shrink it. Read the brand posts for WHICH devices are theirs; their feed can be denser than this ad because a feed post has no headline competing for the eye.",
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (mode === "compose") {
      ...
      // Sem cena derivada, NAO busque foto. A foto do Pexels e reproduzida fielmente (castingRef
      // desligado), entao ela vira o anuncio: assunto, enquadramento e ate a paleta. Buscar a partir
      // dos fatos crus quando a cena falhou foi o que produziu um muro com trepadeira e uma camera
      // de vigilancia numa campanha de gestao de redes sociais (run 304), com a cor indo atras da
      // foto. Sem cena e melhor cair no mundo da propria marca (posts + site), que erra o assunto
      // mas nao erra a identidade.
      const skipPexels = explicitBgSource === "creative" || explicitBgSource === "shapes" || !themeScene;
      ...
      if (isAbVisual) {
        ...
        for (const aspectRatio of uniqueRatios) {
          ...
          const geminiOverlay = bgForZone
            ...
            : null;
          bgByRatio.set(aspectRatio, { url: bgHosted, bgDiag, overlayDiag: (geminiOverlay ? "ok" : (overlayDiag.reason || "nulo-sem-motivo")), rec: gen?.rec ?? null, prompt: bgPrompt, refCount: bgRefImages.length, layout: calm?.layout ?? layoutHint, overlayHtml: geminiOverlay, overlayDiag: overlayDiag.reason });
        }
        ...
        const composeFns = imageTasks.map((task, taskIndex) => async () => {
          ...
          return {
            ...
            height: format.height || 1080,
            variant: variantLabel || null,
            ...(debug ? { debug: { mode: "compose", model: GEMINI_IMAGE_MODELS[0] || null, overlayFonte: (bg as any).overlayHtml ? "modelo" : "template-fixo", overlayMotivo: (bg as any).overlayDiag ?? "?", bgValidate: (bg as any).bgDiag ?? "n/a", bgSource, layout: layoutHint, overlayFromGemini: Boolean(bg.overlayHtml), overlayDiag: bg.overlayDiag ?? null, aspectRatio, prompt: bg.prompt || "", bgRefImagesSent: bg.refCount || 0, composeCompanyRefs: ((campaignData as any).composeCompanyRefs || []), refImagesForGenCount: refImagesForGen.length, refs: refDebug, storeBriefUsed: Boolean(visualBrief && !String((campaignData as any).brandVisualBrief || "").trim()), note: "Logo & copy are composited on top afterwards — not drawn by the image model." } } : {}),
          };
        });
