...
async function buildOverlayHtmlFromGemini(
  ...
  const USER = [
    ...
    `   • CTA: a STANDALONE absolute element (z-index:26), styled with the CTA treatment below. PREFER anchoring it near the BOTTOM of the ad (bottom:6–9%) — the CTA should USUALLY live at the bottom, even when the headline/subheadline sit at the top. Only move it (e.g. just below the subheadline) if the bottom is clearly occupied by the hero/face/product. ⛔ NEVER leave it floating loose in the MIDDLE of the image.`,
    "",
    ...
    "• CTA div: position:absolute; z-index:26 — standalone, NOT a child of the text block. PREFER bottom:6–9% (the CTA usually lives at the bottom of the ad).",
    "• ⛔ NO OVERLAP — this is critical: the logo, headline, subheadline and CTA must EACH occupy their own clear space with breathing room between them. No element may overlap, touch or sit on top of another, and none may cover the visual hero's face. The standalone CTA should sit near the BOTTOM in clear space; never leave it floating in the middle of the image. If space is tight, shrink fonts or tighten gaps, but never let two elements collide.",
    "",
    "RETURN STRUCTURE:",
