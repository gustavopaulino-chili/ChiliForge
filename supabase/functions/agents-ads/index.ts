...
function buildBackgroundPrompt(
  ...
  if (heroRef && hasRefImages) {
    sourceBlock = [
      ...
      "• ⭐⭐ THE PERSON IS CASTING, NOT THE PRODUCT — WE ARE SELLING THE CAMPAIGN'S TOPIC, NOT THIS PERSON: their job is to be a believable face for whatever concrete action the campaign topic (see CAMPAIGN CONTEXT) implies. Do NOT default to a generic 'person looking thoughtfully at a tablet/chart' pose — that could illustrate literally any business and fails the whole point of casting them. Figure out the ONE specific action/prop/pose that makes THIS topic instantly recognizable and put THEM in it. Example: for a TikTok/social-video/viral-growth service → them actively filming with a phone (holding it up at arm's length or on a small tripod), a ring light nearby, mid-take energy — NOT calmly reading a tablet. For a fitness product → mid-workout using it. For a financial app → looking DOWN at a phone held normally in front of them, thumb tapping the screen, in a real moment (paying, transferring, checking a balance) — relaxed body language, NOT arm extended filming. The prop AND the pose/body-language must both be the SPECIFIC thing this topic is about.",
      "⛔⛔ DO NOT REUSE THE SAME POSE ACROSS DIFFERENT TOPICS: an 'arm extended, holding phone up at eye level, filming/selfie' pose is ONLY correct for a content-creation/social-media/TikTok topic. If THIS campaign's topic is something else (finance, fitness, food, education, retail, etc.), that exact pose is WRONG regardless of how well it worked for a different topic — invent the pose that specific topic actually calls for from scratch, using the person naturally, normally, doing that topic's real action (e.g. glancing down at a phone held at chest/waist height, sitting at a desk, using a physical product with their hands, mid-activity) rather than recycling a filming gesture.",
