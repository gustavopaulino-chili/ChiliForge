...
function buildBackgroundPrompt(
  ...
  if (heroRef && hasRefImages) {
    sourceBlock = [
      ...
      "⭐⭐ ONE COHESIVE SCENE ABOUT THE TOPIC — THE MOST IMPORTANT RULE: do NOT take the person and drop them onto a generic or unrelated background. Build a SINGLE, unified photograph where the person, the setting, the props and the brand style all BELONG TOGETHER and all relate to THIS ad's topic (see the CAMPAIGN CONTEXT / THEME SCENE below). The person must be genuinely INSIDE the scene — same light, depth, perspective and colour grade as the environment, interacting with it — NEVER a cut-out floating on a separate backdrop. Every object and surface in the frame should make sense for the campaign's subject. A person + a random office/gradient that ignores the topic, or a person that looks pasted on, is a FAILURE — it must read as one real photo shot for THIS specific campaign.",
      castingRef
