...
import { ArrowLeft, ArrowRight, Sparkles, Copy, Check, ExternalLink, Loader2, Wand2, Link2, RotateCcw, Clock, Download, FileCode, FolderOpen } from 'lucide-react';
...
const Index = () => {
  ...
  if (showResults && generatedFiles.length > 0) {
    const handleDownloadZip = async () => {
      ...
      // Add a README
      zip.file('README.md', `# ${formData.businessName || 'Landing Page'}\n\nGenerated by ChiliForge.\n\n## Getting Started\n\n\`\`\`bash\nnpm install\nnpm run dev\n\`\`\`\n\nOpen [http://localhost:5173](http://localhost:5173) to see your landing page.\n`);
    ...
    return (
      <div className="min-h-screen bg-background relative flex flex-col">
        <div className="reactive-bg-mouse" />
        <Header onLogoClick={() => setShowLanding(true)} />
        <main className="flex-1 flex flex-col mx-auto max-w-4xl w-full px-6 py-8 relative z-10">
          <div className="text-center mb-8">
            ...
            <p className="mt-2 text-muted-foreground text-sm max-w-xl mx-auto">
              Your landing page was generated as a complete React + Vite + Tailwind project.
              Download the ZIP and run <code className="text-xs bg-muted px-1.5 py-0.5 rounded font-mono">npm install && npm run dev</code> to preview.
            </p>
          </div>

          {/* Download button */}
          <div className="flex justify-center mb-8">
            <Button variant="gradient" size="lg" onClick={handleDownloadZip} className="gap-3 text-lg px-8 py-6">
          ...
          <div className="glass-card rounded-xl p-6">
            <h3 className="form-section-title mb-4 flex items-center gap-2">
              ...
              Project Files ({generatedFiles.length} files)
            </h3>
            <div className="space-y-1 max-h-80 overflow-y-auto">
              {generatedFiles.map((file, i) => (
                <div key={i} className="flex items-center gap-2 px-3 py-1.5 rounded-md hover:bg-muted/50 transition-colors">
          ...
          <div className="mt-6 flex gap-3">
            ...
            <div className="ml-auto">
              <Button variant="outline" size="lg" onClick={handleDownloadZip} className="gap-2">
