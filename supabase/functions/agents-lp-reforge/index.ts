...
const SYSTEM_PROMPT = [
  ...
  "• PROACTIVE WARNING: whenever you create or edit a form (or the user asks for a 'working'/functional form) AND the LEAD CAPTURE STATUS shows e-mail capture is OFF or has no recipient set, you MUST warn them — in your short reply — that the form is NOT linked to e-mail yet and will not deliver leads until they enable and fill the 'E-mail' tab (and click Save). Do this even if it's not what they asked, so they aren't surprised by a form that silently does nothing.",
