  ...
  if (bgSource === "reference" && hasRefImages) {
    ...
    if (brandPostsAreProxy) {
      sourceBlock = [
        ...
        hasSiteIdentity
          ? `The FIRST ${Math.max(1, siteRefCount)} attached image(s) are the CLIENT'S OWN WEBSITE — the ONLY source of this brand's COLOUR, mood, DECORATION LEVEL and identity. Colour-grade the WHOLE scene to match them and match the client site's OWN visual richness: if the site is clean/minimal, keep this ad clean/minimal. The OTHER attached images are MARKET/COMPETITOR references: borrow ONLY their high-level COMPOSITION and LAYOUT (where the hero sits, visual hierarchy, use of negative space). Take NONE of their colour, palette, logo, wordmark or identity, and do NOT reproduce their graphic design-asset layer.`
          : "The attached images are MARKET/COMPETITOR references, NOT this brand's own posts. Borrow ONLY their high-level COMPOSITION and LAYOUT (hero placement, hierarchy, negative space). Take NONE of their colour, palette, logo, identity or decoration — colour comes only from the BRAND COLORS line below, and keep the background CLEAN.",
        "⛔ DECORATION OVERRIDE: IGNORE any instruction below to 'match the posts' decoration density', 'camouflage into their feed', or 'reproduce the same busy floating design-asset layer' (floating dots, halftone, blobs, colour panels, diagonal fields, rings, stars, sparkles, badges, icons, confetti). Those instructions assume the attached posts are THIS brand's own — in PROXY MODE they are a COMPETITOR's, so copying their floating graphic overlay is WRONG. That pasted-on 'graphic sticker / CSS-overlay' look is EXACTLY what to avoid. Do NOT add a floating design-asset layer at all unless the CLIENT SITE itself clearly uses one; default to a clean, photographic, uncluttered background.",
        "So when the block below says to keep 'the brand's' palette / grading / identity / decoration density, that means the CLIENT SITE (or the BRAND COLORS line) — NEVER the competitor posts.",
        "",
        sourceBlock,
      ].join("\n");
    }
